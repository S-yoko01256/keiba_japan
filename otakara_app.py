...
# --- 1. 設定：MASTER_LIST（ご提示いただいた正しい値に固定しました） ---
MASTER_LIST = {
...
def get_driver():
    ...
    # 事実：環境内のChromiumパスを明示
    options.binary_location = "/usr/bin/chromium"
    
    # 事実：タイムアウト対策（画像を読み込まない、文字が出たら解析開始）
    options.add_experimental_option("prefs", {"profile.managed_default_content_settings.images": 2})
    options.page_load_strategy = 'eager'
    
    # 修正の事実：Chromium 144系に合致するドライバを強制インストールして同期
                ...
                for r in range(1, 13):
                    ...
                    try:
                        ...
                        if track not in MASTER_LIST[queue['name']]:
                            detailed_logs.append(f"🚫 {queue['name']}{r}R: {track}はMASTER_LIST対象外")
                            continue
                            ...
                            if n_match:
                                ...
                                if cur_ninki in target_ninkis:
                                    ...
                                    if pass_matches and pass_matches[0].startswith("1-"):
                                        ...
                                        detailed_logs.append(f"❌ 脚質NG: {int(cur_ninki)}人気馬 ({h_name if 'h_name' in locals() else ''})")

                    except Exception as e:
                        st.error(f"⚠️ {queue['name']}{r}R でエラー: {e}")
                        driver.quit()
                        driver = get_driver()
